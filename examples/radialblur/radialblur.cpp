...
public:
	...
	// We use an 8 bit per component RGBA offscreen m_vkImage for storing the scene parts that will be blurred
	...
	VulkanExample() : VulkanExampleBase()
	{
		...
		camera.setPerspective(45.0f, (float)m_drawAreaWidth / (float)m_drawAreaHeight, 1.0f, 256.0f);
		timerSpeed *= 0.5f;
	}
	...
	void loadAssets()
	{
		scene.loadFromFile(getAssetPath() + "models/glowsphere.gltf", m_pVulkanDevice, m_vkQueue, vkglTF::FileLoadingFlags::PreTransformVertices | vkglTF::FileLoadingFlags::PreMultiplyVertexColors | vkglTF::FileLoadingFlags::FlipY);
		gradientTexture.loadFromFile(getAssetPath() + "textures/particle_gradient_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, m_pVulkanDevice, m_vkQueue);
	}
	...
	void setupDescriptors()
	{
		...
		// Fullscreen radial blur
		descriptorSetAllocInfo = vks::initializers::descriptorSetAllocateInfo(m_vkDescriptorPool, &descriptorSetLayouts.radialBlur, 1);
	...
	void preparePipelines()
	{
		...
		VkGraphicsPipelineCreateInfo pipelineCI = vks::initializers::pipelineCreateInfo(pipelineLayouts.radialBlur, m_vkRenderPass, 0);
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkDevice, m_vkPipelineCache, 1, &pipelineCI, nullptr, &pipelines.radialBlur));

		// No blending (for debug display)
		blendAttachmentState.blendEnable = VK_FALSE;
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkDevice, m_vkPipelineCache, 1, &pipelineCI, nullptr, &pipelines.offscreenDisplay));
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkDevice, m_vkPipelineCache, 1, &pipelineCI, nullptr, &pipelines.colorPass));
	}
	...
	void prepareUniformBuffers()
	{
		...
		VK_CHECK_RESULT(m_pVulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.blurParams, sizeof(UniformDataBlurParams), &uniformDataBlurParams));
		// Map persistent
	...
	{
		uniformDataScene.projection = glm::perspective(glm::radians(45.0f), (float)m_drawAreaWidth / (float)m_drawAreaHeight, 1.0f, 256.0f);
		camera.setRotation(camera.rotation + glm::vec3(0.0f, m_frameTimer * 10.0f, 0.0f));
		...
		if (!paused) {
			uniformDataScene.gradientPos += m_frameTimer * 0.1f;
		}
