...
public:
	...
	{
            if (m_deviceOriginal) {
                vkDestroyPipeline(m_deviceOriginal, pipelines.logos, nullptr);
                vkDestroyPipeline(m_deviceOriginal, pipelines.models, nullptr);
	...
	void loadAssets()
	{
		...
		for (auto i = 0; i < modelFiles.size(); i++) {
			...
			model.glTF->loadFromFile(getAssetPath() + "models/" + modelFiles[i], m_pVulkanDevice, m_vkQueue, glTFLoadingFlags);
			demoModels.push_back(model);
		}
		// Textures
		skybox.loadFromFile(getAssetPath() + "textures/cubemap_vulkan.ktx", VK_FORMAT_R8G8B8A8_UNORM, m_pVulkanDevice, m_vkQueue);
	}
	...
	void preparePipelines()
	{
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_deviceOriginal, m_vkPipelineCache, 1, &pipelineCI, nullptr, &pipelines.models));
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_deviceOriginal, m_vkPipelineCache, 1, &pipelineCI, nullptr, &pipelines.logos));
	...
	{
		m_pVulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,&uniformBuffer, sizeof(uniformData));
