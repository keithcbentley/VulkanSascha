...
public:
	...
	VulkanExample() : VulkanExampleBase()
	{
		...
		camera.setPerspective(splitScreen ? 30.0f : 45.0f, (float)m_drawAreaWidth / (float)(m_drawAreaHeight * ((splitScreen) ? 0.5f : 1.0f)), 1.0f, 256.0f);
	}
	...
	~VulkanExample()
	{
		if (m_vkDevice) {
			...
			vkDestroyPipeline(m_vkDevice, pipelines.sdf, nullptr);
			vkDestroyPipeline(m_vkDevice, pipelines.bitmap, nullptr);
	...
	void parsebmFont()
	{
		...
		while (!istream.eof())
		{
			...
			if (info == "char")
			{
				...
				// Char m_vkPhysicalDeviceProperties
	...
	void loadAssets()
	{
		textures.fontSDF.loadFromFile(getAssetPath() + "textures/font_sdf_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, m_pVulkanDevice, m_vkQueue);
		textures.fontBitmap.loadFromFile(getAssetPath() + "textures/font_bitmap_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, m_pVulkanDevice, m_vkQueue);
	}
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			VkViewport viewport = vks::initializers::viewport((float)m_drawAreaWidth, (splitScreen) ? (float)m_drawAreaHeight / 2.0f : (float)m_drawAreaHeight, 0.0f, 1.0f);
			...
			vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, m_vkPipelineLayout, 0, 1, &descriptorSets.sdf, 0, NULL);
			...
			if (splitScreen)
			{
				viewport.y = (float)m_drawAreaHeight / 2.0f;
				vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);
				vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, m_vkPipelineLayout, 0, 1, &descriptorSets.bitmap, 0, NULL);
	...
	void generateText(std:: string text)
	{
		...
		VK_CHECK_RESULT(m_pVulkanDevice->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &vertexBuffer, vertices.size() * sizeof(Vertex), vertices.data()));
		VK_CHECK_RESULT(m_pVulkanDevice->createBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &indexBuffer, indices.size() * sizeof(uint32_t), indices.data()));
	}
	...
	void setupDescriptors()
	{
		...
		VK_CHECK_RESULT(vkAllocateDescriptorSets(m_vkDevice, &allocInfo, &descriptorSets.sdf));
		...
		VK_CHECK_RESULT(vkAllocateDescriptorSets(m_vkDevice, &allocInfo, &descriptorSets.bitmap));
		writeDescriptorSets = {
	...
	void preparePipelines()
	{
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkDevice, m_vkPipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.sdf));
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkDevice, m_vkPipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.bitmap));
	}
