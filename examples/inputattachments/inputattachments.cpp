...
public:
    ...
    void createAttachment(VkFormat format, VkImageUsageFlags usage, FrameBufferAttachment* attachment)
    {
        ...
        vkcpp::ImageCreateInfo imageCreateInfo(format, usage | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT);
        ...
        attachment->m_image = vkcpp::Image(imageCreateInfo);

        VkMemoryRequirements vkMemoryRequirements = attachment->m_image.getMemoryRequirements();
        attachment->m_deviceMemory = vkcpp::DeviceMemory(vkMemoryRequirements, vkcpp::MEMORY_PROPERTY_DEVICE_LOCAL);
        attachment->m_image.bindImageMemory(attachment->m_deviceMemory);

        vkcpp::ImageViewCreateInfo imageViewCreateInfo
            = vkcpp::ImageViewCreateInfo(
                attachment->m_image,
                VK_IMAGE_VIEW_TYPE_2D,
                format,
                aspectMask);
        attachment->m_imageView = vkcpp::ImageView(imageViewCreateInfo);
    }
    ...
    void setupFrameBuffer()
    {
        ...
        for (uint32_t i = 0; i < m_vkFrameBuffers.size(); i++) {
            ...
            views[1] = attachments[i].color.m_imageView;
            views[2] = attachments[i].depth.m_imageView;
