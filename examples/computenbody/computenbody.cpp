/*
 * Vulkan Example - Compute shader N-body simulation using two passes and shared compute shader m_vkDeviceMemory
 *
 ...
 * To optimize performance, the compute shaders use shared m_vkDeviceMemory
 *
...
public:
    ...
    void buildComputeCommandBuffer()
    {
        vkcpp::CommandBuffer commandBuffer
            = vkcpp::CommandBuffer::makeCopy(compute.commandBuffer);
        commandBuffer.begin();
        ...
        // -------------------------------------------------------------------------------------------------------
        commandBuffer.cmdBindComputePipeline(compute.pipelineIntegrate);
        commandBuffer.cmdDispatch(numParticles / 256, 1, 1);
    ...
    void prepareStorageBuffers()
    {
        ...
        VkCommandBuffer vkCommandBuffer = m_pVulkanDevice->createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
        VkBufferCopy copyRegion = {};
        copyRegion.size = storageBufferSize;
        vkCmdCopyBuffer(vkCommandBuffer, stagingBuffer.m_buffer, storageBuffer.m_buffer, 1, &copyRegion);
        ...
        m_pVulkanDevice->flushCommandBuffer(vkCommandBuffer, m_queue, true);
    ...
    void prepareGraphics()
    {
        ...
        VK_CHECK_RESULT(vkAllocateDescriptorSets(m_device, &allocInfo, &graphics.descriptorSet));

        std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
            vks::initializers::writeDescriptorSet(
				graphics.descriptorSet,
				VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
				0, &textures.particle.m_vkDescriptorImageInfo),
            vks::initializers::writeDescriptorSet(
				graphics.descriptorSet,
				VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
				1, &textures.gradient.m_vkDescriptorImageInfo),
            vks::initializers::writeDescriptorSet(
				graphics.descriptorSet,
				VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
				2, &graphics.uniformBuffer.m_vkDescriptorBufferInfo),
        };
        ...
        VK_CHECK_RESULT(vkCreateSemaphore(
			m_device, &semaphoreCreateInfo, nullptr, &graphics.semaphore));
    ...
    void prepareCompute()
    {
        ...
        VkDescriptorSetAllocateInfo allocInfo
			= vks::initializers::descriptorSetAllocateInfo(m_descriptorPool, &compute.descriptorSetLayout, 1);
        VK_CHECK_RESULT(vkAllocateDescriptorSets(m_device, &allocInfo, &compute.descriptorSet));
        ...
        VK_CHECK_RESULT(vkCreatePipelineLayout(m_device, &pipelineLayoutCreateInfo, nullptr, &compute.pipelineLayout));
        ...
        // We want to use as much shared m_vkDeviceMemory for the compute shader invocations as available, so we calculate it based on the m_vkDevice limits and pass it to the shader via specialization constants
        ...
        VK_CHECK_RESULT(vkCreateComputePipelines(
			m_device, m_vkPipelineCache, 1, &computePipelineCreateInfo, nullptr, &compute.pipelineCalculate));
        ...
        VK_CHECK_RESULT(vkCreateComputePipelines(
			m_device, m_vkPipelineCache, 1, &computePipelineCreateInfo, nullptr, &compute.pipelineIntegrate));
