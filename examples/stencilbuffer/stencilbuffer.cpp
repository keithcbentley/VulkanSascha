...
public:
    ...
    void buildCommandBuffers()
    {
        ...
        for (int32_t i = 0; i < m_drawCommandBuffers.size(); ++i) {
            vkcpp::CommandBuffer commandBuffer = vkcpp::CommandBuffer::makeCopy(
            ...
            commandBuffer.cmdBindVertexBuffer(model.vertices.buffer);
            commandBuffer.cmdBindIndexBuffer(model.indices.buffer, VK_INDEX_TYPE_UINT32);
            ...
            commandBuffer.cmdBindPipeline(pipelines.stencil);
            model.draw(commandBuffer);

            // Second pass renders scaled object only where stencil was not set by first pass
            commandBuffer.cmdBindPipeline(pipelines.outline);
            model.draw(commandBuffer);
