...
public:
    ...
    {
        if (m_device) {
            vkDestroyPipeline(m_device, pipelines.MSAA, nullptr);
            vkDestroyPipeline(m_device, pipelines.MSAASampleShading, nullptr);
            ...
            // Destroy MSAA target
            vkDestroyImage(m_device, multisampleTarget.color.image, nullptr);
            vkDestroyImageView(m_device, multisampleTarget.color.view, nullptr);
            vkFreeMemory(m_device, multisampleTarget.color.memory, nullptr);
    ...
    setupMultisampleTarget()
    {
        ...
        VK_CHECK_RESULT(vkCreateImage(m_device, &info, nullptr, &multisampleTarget.color.image));
        ...
        VK_CHECK_RESULT(vkCreateImageView(m_device, &viewInfo, nullptr, &multisampleTarget.color.view));
        ...
        VK_CHECK_RESULT(vkCreateImage(m_device, &info, nullptr, &multisampleTarget.depth.image));

        vkGetImageMemoryRequirements(m_device, multisampleTarget.depth.image, &memReqs);
        ...
        VK_CHECK_RESULT(vkCreateImageView(m_device, &viewInfo, nullptr, &multisampleTarget.depth.view));
    }
    ...
    void buildCommandBuffers()
    {
        ...
        for (int32_t i = 0; i < m_drawCommandBuffers.size(); ++i) {
            ...
            vkCmdBindPipeline(m_drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, useSampleShading ? pipelines.MSAASampleShading : pipelines.MSAA);
            model.draw(m_drawCommandBuffers[i], vkglTF::RenderFlags::BindImages, m_vkPipelineLayout);
    ...
    void loadAssets()
    {
        model.loadFromFile(getAssetPath() + "models/voyager.gltf", m_pVulkanDevice, m_queue, vkglTF::FileLoadingFlags::PreTransformVertices | vkglTF::FileLoadingFlags::FlipY);
    }
    ...
    void preparePipelines()
    {
        ...
        VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_device, m_vkPipelineCache, 1, &pipelineCI, nullptr, &pipelines.MSAA));
        ...
        //		if (m_pVulkanDevice->m_vkPhysicalDeviceFeatures.sampleRateShading)
        {
            ...
            VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_device, m_vkPipelineCache, 1, &pipelineCI, nullptr, &pipelines.MSAASampleShading));
        }
    }
