/*
* Vulkan Example - Physical based rendering a textured object (metal/roughness workflow) with m_vkImage based lighting
*
...
public:
	...
	void buildCommandBuffers()
	{
		...
		for (size_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			if (displaySkybox)
			{
				vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, m_vkPipelineLayout, 0, 1, &descriptorSets.skybox, 0, NULL);
				vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.skybox);
				models.skybox.draw(drawCmdBuffers[i]);
			}

			// Objects
			vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, m_vkPipelineLayout, 0, 1, &descriptorSets.object, 0, NULL);
	...
	void loadAssets()
	{
		...
		models.object.loadFromFile(getAssetPath() + "models/cerberus/cerberus.gltf", m_pVulkanDevice, m_vkQueue, glTFLoadingFlags);
		textures.environmentCube.loadFromFile(getAssetPath() + "textures/hdr/gcanyon_cube.ktx", VK_FORMAT_R16G16B16A16_SFLOAT, m_pVulkanDevice, m_vkQueue);
		textures.albedoMap.loadFromFile(getAssetPath() + "models/cerberus/albedo.ktx", VK_FORMAT_R8G8B8A8_UNORM, m_pVulkanDevice, m_vkQueue);
		textures.normalMap.loadFromFile(getAssetPath() + "models/cerberus/normal.ktx", VK_FORMAT_R8G8B8A8_UNORM, m_pVulkanDevice, m_vkQueue);
		textures.aoMap.loadFromFile(getAssetPath() + "models/cerberus/ao.ktx", VK_FORMAT_R8_UNORM, m_pVulkanDevice, m_vkQueue);
		textures.metallicMap.loadFromFile(getAssetPath() + "models/cerberus/metallic.ktx", VK_FORMAT_R8_UNORM, m_pVulkanDevice, m_vkQueue);
		textures.roughnessMap.loadFromFile(getAssetPath() + "models/cerberus/roughness.ktx", VK_FORMAT_R8_UNORM, m_pVulkanDevice, m_vkQueue);
	}
	...
	void preparePipelines()
	{
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkDevice, m_vkPipelineCache, 1, &pipelineCI, nullptr, &pipelines.pbr));
	}
	...
	void generateBRDFLUT()
	{
		...
		VK_CHECK_RESULT(vkCreateImage(m_vkDevice, &imageCI, nullptr, &textures.lutBrdf.image));
		...
		VK_CHECK_RESULT(vkCreateImageView(m_vkDevice, &viewCI, nullptr, &textures.lutBrdf.view));
		// Sampler
		...
		VK_CHECK_RESULT(vkCreateSampler(m_vkDevice, &samplerCI, nullptr, &textures.lutBrdf.sampler));
		...
		VK_CHECK_RESULT(vkCreateFramebuffer(m_vkDevice, &framebufferCI, nullptr, &framebuffer));
		...
		VkPipeline pipeline;
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_vkDevice, m_vkPipelineCache, 1, &pipelineCI, nullptr, &pipeline));
	...
	void generateIrradianceCube()
	{
		...
		VK_CHECK_RESULT(vkCreateImage(m_vkDevice, &imageCI, nullptr, &textures.irradianceCube.image));
		VkMemoryAllocateInfo memAlloc = vks::initializers::memoryAllocateInfo();
		VkMemoryRequirements memReqs;
		...
		VK_CHECK_RESULT(vkCreateImageView(m_vkDevice, &viewCI, nullptr, &textures.irradianceCube.view));
		// Sampler
		VkSamplerCreateInfo samplerCI = vks::initializers::samplerCreateInfo();
		samplerCI.magFilter = VK_FILTER_LINEAR;
		samplerCI.minFilter = VK_FILTER_LINEAR;
		...
		samplerCI.borderColor = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;
		VK_CHECK_RESULT(vkCreateSampler(m_vkDevice, &samplerCI, nullptr, &textures.irradianceCube.sampler));
		...
		// Offfscreen framebuffer
		{
			...
			VK_CHECK_RESULT(vkCreateImage(m_vkDevice, &imageCreateInfo, nullptr, &offscreen.image));
			...
			VK_CHECK_RESULT(vkCreateImageView(m_vkDevice, &colorImageView, nullptr, &offscreen.view));
		...
		// Change m_vkImage layout for all cubemap faces to transfer destination
		vks::tools::setImageLayout(
			cmdBuf,
		...
		vkFreeMemory(m_vkDevice, offscreen.memory, nullptr);
		vkDestroyImageView(m_vkDevice, offscreen.view, nullptr);
		vkDestroyImage(m_vkDevice, offscreen.image, nullptr);
		vkDestroyDescriptorPool(m_vkDevice, descriptorpool, nullptr);
		vkDestroyDescriptorSetLayout(m_vkDevice, descriptorsetlayout, nullptr);
		vkDestroyPipeline(m_vkDevice, pipeline, nullptr);
		vkDestroyPipelineLayout(m_vkDevice, pipelinelayout, nullptr);
	...
	// See https://placeholderart.wordpress.com/2015/07/28/implementation-notes-runtime-environment-map-filtering-for-m_vkImage-based-lighting/
	void generatePrefilteredCube()
	{
		...
		imageCI.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
		imageCI.flags = VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT;
		VK_CHECK_RESULT(vkCreateImage(m_vkDevice, &imageCI, nullptr, &textures.prefilteredCube.image));
		VkMemoryAllocateInfo memAlloc = vks::initializers::memoryAllocateInfo();
		VkMemoryRequirements memReqs;
		...
		VK_CHECK_RESULT(vkCreateImageView(m_vkDevice, &viewCI, nullptr, &textures.prefilteredCube.view));
		// Sampler
		VkSamplerCreateInfo samplerCI = vks::initializers::samplerCreateInfo();
		samplerCI.magFilter = VK_FILTER_LINEAR;
		samplerCI.minFilter = VK_FILTER_LINEAR;
		...
		samplerCI.maxLod = static_cast<float>(numMips);
		samplerCI.borderColor = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;
		VK_CHECK_RESULT(vkCreateSampler(m_vkDevice, &samplerCI, nullptr, &textures.prefilteredCube.sampler));
