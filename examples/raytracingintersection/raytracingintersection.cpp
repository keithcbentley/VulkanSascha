...
public:
	...
	void createBuffers()
	{
		...
		// Copy the buffer to the m_vkDevice for performance reasons
		...
		// Spheres
		VK_CHECK_RESULT(m_pVulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffer, sizeof(Sphere)* spheres.size(), spheres.data()));
		VK_CHECK_RESULT(m_pVulkanDevice->createBuffer(usageFlags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &spheresBuffer, sizeof(Sphere)* spheres.size()));
		m_pVulkanDevice->copyBuffer(&stagingBuffer, &spheresBuffer, m_vkQueue);
		stagingBuffer.destroy();

		// AABBs
		VK_CHECK_RESULT(m_pVulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffer, sizeof(AABB)* aabbs.size(), aabbs.data()));
		VK_CHECK_RESULT(m_pVulkanDevice->createBuffer(usageFlags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &aabbsBuffer, sizeof(AABB)* aabbs.size()));
		m_pVulkanDevice->copyBuffer(&stagingBuffer, &aabbsBuffer, m_vkQueue);
		stagingBuffer.destroy();
	}
	...
	*/
	void createDescriptorSets()
	{
		...
		// We pass the sphere descriptions as shader storage buffer, so the ray tracing shaders can source m_vkPhysicalDeviceProperties from it
