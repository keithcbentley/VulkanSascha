 ...
 *
 * This samples shows how to generate a full mip-chain from a top-level m_vkImage and how different sampling modes compare
 *
...
public:
    ...
    // Loads a full sized m_vkImage from disk, generates a Vulkan m_vkImage (texture) from it and creates a full mip chain using blits
    void loadTextureAndGenerateMips(std::string filename, VkFormat format)
    {
        ...
        memAllocInfo.allocationSize = memReqs.size;
        memAllocInfo.memoryTypeIndex = vkcpp::findMemoryTypeIndex(memReqs.memoryTypeBits, vkcpp::MEMORY_PROPERTY_DEVICE_LOCAL);
        ...
        // Optimal m_vkImage will be used as destination for the copy, so we must transfer from our initial undefined m_vkImage layout to the transfer destination layout
        ...
        for (uint32_t i = 1; i < texture.mipLevels; i++) {
            ...
            // Prepare current mip level as m_vkImage blit destination
            vks::tools::insertImageMemoryBarrier(
                blitCmd,
                texture.image,
                0,
            ...
            // Prepare current mip level as m_vkImage blit source for next level
            vks::tools::insertImageMemoryBarrier(
                blitCmd,
                texture.image,
                VK_ACCESS_TRANSFER_WRITE_BIT,
        ...
        m_pVulkanDevice->flushCommandBuffer(blitCmd, m_queue, true);
        // ---------------------------------------------------------------
        ...
        // Without mip mapping
        VK_CHECK_RESULT(vkCreateSampler(m_device, &sampler, nullptr, &samplers[0]));

        // With mip mapping
        sampler.maxLod = (float)texture.mipLevels;
        VK_CHECK_RESULT(vkCreateSampler(m_device, &sampler, nullptr, &samplers[1]));

        // With mip mapping and anisotropic filtering
        if (vkcpp::vkPhysicalDeviceFeatures().samplerAnisotropy) {
            sampler.maxAnisotropy = vkcpp::vkPhysicalDeviceProperties().limits.maxSamplerAnisotropy;
            sampler.anisotropyEnable = VK_TRUE;
        }
        VK_CHECK_RESULT(vkCreateSampler(m_device, &sampler, nullptr, &samplers[2]));
    ...
    void destroyTextureImage(Texture texture)
    {
        ...
        vkFreeMemory(m_device, texture.deviceMemory, nullptr);
    }
    ...
    void loadAssets()
    {
        model.loadFromFile(getAssetPath() + "models/tunnel_cylinder.gltf", m_pVulkanDevice, m_queue, vkglTF::FileLoadingFlags::PreTransformVertices | vkglTF::FileLoadingFlags::FlipY);
    ...
    void setupDescriptors()
    {
        ...
        std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
            ...
            vks::initializers::writeDescriptorSet(
				...
				0, &uniformBuffer.m_vkDescriptorBufferInfo),
            // Binding 1: Sampled m_vkImage
