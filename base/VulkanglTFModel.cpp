...
/*
 * Note that this isn't a complete glTF loader and not all m_vkPhysicalDeviceFeatures of the glTF 2.0 spec are supported
...
void vkglTF::Texture::destroy()
{
    if (device) {
        ...
        vkFreeMemory(device->m_device, deviceMemory, nullptr);
...
void vkglTF::Texture::fromglTfImage(tinygltf::Image& gltfimage, std::string path, vks::VulkanDevice* device, VkQueue copyQueue)
{
    ...
    if (!isKtx) {
        ...
        uint8_t* data;
        VK_CHECK_RESULT(vkMapMemory(device->m_device, stagingMemory, 0, memReqs.size, 0, (void**)&data));
        memcpy(data, buffer, bufferSize);
        vkUnmapMemory(device->m_device, stagingMemory);
        ...
        vkFreeMemory(device->m_device, stagingMemory, nullptr);
        ...
        // Get device m_vkPhysicalDeviceProperties for the requested texture format
        VkFormatProperties formatProperties;
        vkGetPhysicalDeviceFormatProperties(device->m_physicalDevice, format, &formatProperties);
    ...
    VK_CHECK_RESULT(vkCreateImageView(device->m_device, &viewInfo, nullptr, &view));
...
void vkglTF::Material::createDescriptorSet(VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout, uint32_t descriptorBindingFlags)
{
    ...
    VK_CHECK_RESULT(vkAllocateDescriptorSets(device->m_device, &descriptorSetAllocInfo, &descriptorSet));
...
vkglTF::Mesh::Mesh(vks::VulkanDevice* device, glm::mat4 matrix)
{
    ...
    VK_CHECK_RESULT(device->createBuffer(
        ...
        &uniformBlock));
    VK_CHECK_RESULT(vkMapMemory(device->m_device, uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped));
    uniformBuffer.descriptor = { uniformBuffer.buffer, 0, sizeof(uniformBlock) };
};

vkglTF::Mesh::~Mesh()
{
    vkDestroyBuffer(device->m_device, uniformBuffer.buffer, nullptr);
    vkFreeMemory(device->m_device, uniformBuffer.memory, nullptr);
...
VkVertexInputBindingDescription vkglTF::Vertex::s_vertexInputBindingDescription;
std::vector<VkVertexInputAttributeDescription> vkglTF::Vertex::s_vertexInputAttributeDescriptions;
VkPipelineVertexInputStateCreateInfo vkglTF::Vertex::s_pipelineVertexInputStateCreateInfo;

VkVertexInputBindingDescription vkglTF::Vertex::s_inputBindingDescription(uint32_t binding)
{
    return VkVertexInputBindingDescription({ binding, sizeof(Vertex), VK_VERTEX_INPUT_RATE_VERTEX });
}

VkVertexInputAttributeDescription vkglTF::Vertex::s_inputAttributeDescription(uint32_t binding, uint32_t location, VertexComponent component)
{
    switch (component) {
...
std::vector<VkVertexInputAttributeDescription> vkglTF::Vertex::s_inputAttributeDescriptions(uint32_t binding, const std::vector<VertexComponent> components)
{
    std::vector<VkVertexInputAttributeDescription> result;
    uint32_t location = 0;
    for (VertexComponent component : components) {
        result.push_back(Vertex::s_inputAttributeDescription(binding, location, component));
        location++;
    }
    return result;
}
...
{
    s_vertexInputBindingDescription = Vertex::s_inputBindingDescription(0);
    Vertex::s_vertexInputAttributeDescriptions = Vertex::s_inputAttributeDescriptions(0, components);
    s_pipelineVertexInputStateCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
    s_pipelineVertexInputStateCreateInfo.vertexBindingDescriptionCount = 1;
    s_pipelineVertexInputStateCreateInfo.pVertexBindingDescriptions = &Vertex::s_vertexInputBindingDescription;
    s_pipelineVertexInputStateCreateInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(Vertex::s_vertexInputAttributeDescriptions.size());
    s_pipelineVertexInputStateCreateInfo.pVertexAttributeDescriptions = Vertex::s_vertexInputAttributeDescriptions.data();
    return &s_pipelineVertexInputStateCreateInfo;
}

vkcpp::PipelineVertexInputStateCreateInfo vkglTF::Vertex::getPipelineVertexInputStateVkcpp(
    const std::vector<VertexComponent> components)
{
    ...
    s_pipelineVertexInputStateCreateInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(Vertex::s_vertexInputAttributeDescriptions.size());
    s_pipelineVertexInputStateCreateInfo.pVertexAttributeDescriptions = Vertex::s_vertexInputAttributeDescriptions.data();
    vkcpp::PipelineVertexInputStateCreateInfo pipelineVertexInputStateCreateInfo;

    pipelineVertexInputStateCreateInfo.addVertexInputBindingDescription(Vertex::s_inputBindingDescription(0));
    for (auto& attribute : Vertex::s_vertexInputAttributeDescriptions) {
        pipelineVertexInputStateCreateInfo.addVertexInputAttributeDescription(attribute);
    }
...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue)
{
    ...
    imageCreateInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
    VK_CHECK_RESULT(vkCreateImage(device->m_device, &imageCreateInfo, nullptr, &emptyTexture.image));
    ...
    VK_CHECK_RESULT(vkCreateSampler(device->m_device, &samplerCreateInfo, nullptr, &emptyTexture.sampler));
    ...
    VK_CHECK_RESULT(vkCreateImageView(device->m_device, &viewCreateInfo, nullptr, &emptyTexture.view));
...
vkglTF::Model::~Model()
{
    vkDestroyBuffer(device->m_device, vertices.buffer, nullptr);
    vkFreeMemory(device->m_device, vertices.memory, nullptr);
    vkDestroyBuffer(device->m_device, indices.buffer, nullptr);
    vkFreeMemory(device->m_device, indices.memory, nullptr);
    ...
    if (descriptorSetLayoutUbo != VK_NULL_HANDLE) {
        vkDestroyDescriptorSetLayout(device->m_device, descriptorSetLayoutUbo, nullptr);
        descriptorSetLayoutUbo = VK_NULL_HANDLE;
    }
    if (descriptorSetLayoutImage != VK_NULL_HANDLE) {
        vkDestroyDescriptorSetLayout(device->m_device, descriptorSetLayoutImage, nullptr);
...
void vkglTF::Model::loadFromFile(std::string filename, vks::VulkanDevice* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
    ...
    // Pre-Calculations for requested m_vkPhysicalDeviceFeatures
    ...
    vkDestroyBuffer(device->m_device, vertexStaging.buffer, nullptr);
    vkFreeMemory(device->m_device, vertexStaging.memory, nullptr);
    vkDestroyBuffer(device->m_device, indexStaging.buffer, nullptr);
    vkFreeMemory(device->m_device, indexStaging.memory, nullptr);
    ...
    VK_CHECK_RESULT(vkCreateDescriptorPool(device->m_device, &descriptorPoolCI, nullptr, &descriptorPool));
    ...
    // Descriptors for per-node uniform buffers
    {
        ...
        if (descriptorSetLayoutUbo == VK_NULL_HANDLE) {
            ...
            VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device->m_device, &descriptorLayoutCI, nullptr, &descriptorSetLayoutUbo));
        }
        for (auto node : nodes) {
    ...
    // Descriptors for per-material images
    {
        ...
        if (descriptorSetLayoutImage == VK_NULL_HANDLE) {
            ...
            descriptorLayoutCI.bindingCount = static_cast<uint32_t>(setLayoutBindings.size());
            descriptorLayoutCI.pBindings = setLayoutBindings.data();
            VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device->m_device, &descriptorLayoutCI, nullptr, &descriptorSetLayoutImage));
        }
        for (auto& material : materials) {
            if (material.baseColorTexture != nullptr) {
...
void vkglTF::Model::prepareNodeDescriptor(vkglTF::Node* node, VkDescriptorSetLayout descriptorSetLayout)
{
    if (node->mesh) {
        ...
        descriptorSetAllocInfo.pSetLayouts = &descriptorSetLayout;
        descriptorSetAllocInfo.descriptorSetCount = 1;
        VK_CHECK_RESULT(vkAllocateDescriptorSets(device->m_device, &descriptorSetAllocInfo, &node->mesh->uniformBuffer.descriptorSet));
        ...
        vkUpdateDescriptorSets(device->m_device, 1, &writeDescriptorSet, 0, nullptr);
    }
    for (auto& child : node->children) {
