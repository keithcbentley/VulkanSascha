...
void vkglTF::Texture::destroy()
{
	if (device)
	{
		vkDestroyImageView(device->m_vkDevice, view, nullptr);
		vkDestroyImage(device->m_vkDevice, image, nullptr);
		vkFreeMemory(device->m_vkDevice, deviceMemory, nullptr);
...
void vkglTF::Texture::fromglTfImage(tinygltf::Image &gltfimage, std::string path, vks::VulkanDevice *device, VkQueue copyQueue)
{
	...
	if (!isKtx) {
		...
		VK_CHECK_RESULT(vkCreateBuffer(device->m_vkDevice, &bufferCreateInfo, nullptr, &stagingBuffer));
		vkGetBufferMemoryRequirements(device->m_vkDevice, stagingBuffer, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		VK_CHECK_RESULT(vkAllocateMemory(device->m_vkDevice, &memAllocInfo, nullptr, &stagingMemory));
		VK_CHECK_RESULT(vkBindBufferMemory(device->m_vkDevice, stagingBuffer, stagingMemory, 0));

		uint8_t* data;
		VK_CHECK_RESULT(vkMapMemory(device->m_vkDevice, stagingMemory, 0, memReqs.size, 0, (void**)&data));
		...
		VK_CHECK_RESULT(vkCreateImage(device->m_vkDevice, &imageCreateInfo, nullptr, &image));
		vkGetImageMemoryRequirements(device->m_vkDevice, image, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		VK_CHECK_RESULT(vkAllocateMemory(device->m_vkDevice, &memAllocInfo, nullptr, &deviceMemory));
		VK_CHECK_RESULT(vkBindImageMemory(device->m_vkDevice, image, deviceMemory, 0));
	...
	VK_CHECK_RESULT(vkCreateImageView(device->m_vkDevice, &viewInfo, nullptr, &view));
...
void vkglTF::Material::createDescriptorSet(VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout, uint32_t descriptorBindingFlags)
{
	...
	VK_CHECK_RESULT(vkAllocateDescriptorSets(device->m_vkDevice, &descriptorSetAllocInfo, &descriptorSet));
...
vkglTF::Mesh::Mesh(vks::VulkanDevice *device, glm::mat4 matrix) {
	...
	VK_CHECK_RESULT(device->createBuffer(
		...
		&uniformBlock));
	VK_CHECK_RESULT(vkMapMemory(device->m_vkDevice, uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped));
	uniformBuffer.descriptor = { uniformBuffer.buffer, 0, sizeof(uniformBlock) };
};

vkglTF::Mesh::~Mesh() {
	vkDestroyBuffer(device->m_vkDevice, uniformBuffer.buffer, nullptr);
	vkFreeMemory(device->m_vkDevice, uniformBuffer.memory, nullptr);
...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue)
{
	...
	imageCreateInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
	VK_CHECK_RESULT(vkCreateImage(device->m_vkDevice, &imageCreateInfo, nullptr, &emptyTexture.image));

	vkGetImageMemoryRequirements(device->m_vkDevice, emptyTexture.image, &memReqs);
	memAllocInfo.allocationSize = memReqs.size;
	memAllocInfo.memoryTypeIndex = device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	VK_CHECK_RESULT(vkAllocateMemory(device->m_vkDevice, &memAllocInfo, nullptr, &emptyTexture.deviceMemory));
	VK_CHECK_RESULT(vkBindImageMemory(device->m_vkDevice, emptyTexture.image, emptyTexture.deviceMemory, 0));
	...
	VK_CHECK_RESULT(vkCreateSampler(device->m_vkDevice, &samplerCreateInfo, nullptr, &emptyTexture.sampler));
	...
	VK_CHECK_RESULT(vkCreateImageView(device->m_vkDevice, &viewCreateInfo, nullptr, &emptyTexture.view));
...
vkglTF::Model::~Model()
{
	vkDestroyBuffer(device->m_vkDevice, vertices.buffer, nullptr);
	vkFreeMemory(device->m_vkDevice, vertices.memory, nullptr);
	vkDestroyBuffer(device->m_vkDevice, indices.buffer, nullptr);
	vkFreeMemory(device->m_vkDevice, indices.memory, nullptr);
	...
	if (descriptorSetLayoutUbo != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(device->m_vkDevice, descriptorSetLayoutUbo, nullptr);
		descriptorSetLayoutUbo = VK_NULL_HANDLE;
	}
	if (descriptorSetLayoutImage != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(device->m_vkDevice, descriptorSetLayoutImage, nullptr);
...
void vkglTF::Model::loadFromFile(std::string filename, vks::VulkanDevice *device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	vkDestroyBuffer(device->m_vkDevice, vertexStaging.buffer, nullptr);
	vkFreeMemory(device->m_vkDevice, vertexStaging.memory, nullptr);
	vkDestroyBuffer(device->m_vkDevice, indexStaging.buffer, nullptr);
	vkFreeMemory(device->m_vkDevice, indexStaging.memory, nullptr);
	...
	VK_CHECK_RESULT(vkCreateDescriptorPool(device->m_vkDevice, &descriptorPoolCI, nullptr, &descriptorPool));
	...
	// Descriptors for per-node uniform buffers
	{
		...
		if (descriptorSetLayoutUbo == VK_NULL_HANDLE) {
			...
			VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device->m_vkDevice, &descriptorLayoutCI, nullptr, &descriptorSetLayoutUbo));
		}
		for (auto node : nodes) {
	...
	// Descriptors for per-material images
	{
		...
		if (descriptorSetLayoutImage == VK_NULL_HANDLE) {
			...
			descriptorLayoutCI.bindingCount = static_cast<uint32_t>(setLayoutBindings.size());
			descriptorLayoutCI.pBindings = setLayoutBindings.data();
			VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device->m_vkDevice, &descriptorLayoutCI, nullptr, &descriptorSetLayoutImage));
		}
		for (auto& material : materials) {
			if (material.baseColorTexture != nullptr) {
...
void vkglTF::Model::prepareNodeDescriptor(vkglTF::Node* node, VkDescriptorSetLayout descriptorSetLayout) {
	if (node->mesh) {
		...
		descriptorSetAllocInfo.pSetLayouts = &descriptorSetLayout;
		descriptorSetAllocInfo.descriptorSetCount = 1;
		VK_CHECK_RESULT(vkAllocateDescriptorSets(device->m_vkDevice, &descriptorSetAllocInfo, &node->mesh->uniformBuffer.descriptorSet));
		...
		vkUpdateDescriptorSets(device->m_vkDevice, 1, &writeDescriptorSet, 0, nullptr);
	}
	for (auto& child : node->children) {
