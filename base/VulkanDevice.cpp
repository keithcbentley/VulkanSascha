    ...
    void* data)
{
	...
	buffer->m_buffer = vkcpp::Buffer(bufferCreateInfo);
	...
	buffer->m_deviceMemory = vkcpp::DeviceMemory(memAlloc);
...
{
    return createCommandBuffer(level, m_graphicsCommandPoolOriginal, begin);
}
...
{
    return flushCommandBuffer(commandBuffer, queue, m_graphicsCommandPoolOriginal, free);
}
